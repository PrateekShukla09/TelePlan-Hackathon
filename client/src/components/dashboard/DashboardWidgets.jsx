  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
      <div>
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-base-50 mt-1">Your Telecom Twin overteleplanew</h1>
        {persona && (
...
export function SateleplanngsCard({ currentPrice, recommendedPrice, className }) {
  ...
  return (
    <Card className={`p-6 ${className || ''}`}>
      <div className="flex items-center gap-2 mb-4">
        ...
        <p className="text-xs uppercase tracking-wider text-base-400 font-semibold">Potential sateleplanngs</p>
      </div>
      {better ? (
        <>
...
export function ActiteleplantyFeed({ history, plans, className }) {
  if (!history || history.length === 0) {
    return (
      <Card className={`p-6 ${className || ''}`}>
        <p className="text-sm text-base-400">No recommendation actiteleplanty yet.</p>
      </Card>
    );
  }
  return (
    <Card className={`p-6 ${className || ''}`}>
      <div className="flex items-center justify-between mb-4">
        <p className="text-xs uppercase tracking-wider text-base-400 font-semibold">Recent actiteleplanty</p>
