...
import { GreetingHeader, TwinSummaryGrid, CurrentPlanCard, SateleplanngsCard, ActiteleplantyFeed } from '@/components/dashboard/DashboardWidgets';
import { UsageOverteleplanewChart } from '@/components/dashboard/UsageOverteleplanewChart';
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <Card className="p-6 lg:col-span-2">
          <div className="flex items-center justify-between mb-2">
            <p className="text-xs uppercase tracking-wider text-base-400 font-semibold">Usage overteleplanew</p>
            <Button as={Link} to="/app/twin" variant="ghost" size="sm" iconRight={ArrowRight}>
              Full Telecom Twin
            </Button>
          </div>
          <UsageOverteleplanewChart usage={customer.usage} />
        </Card>
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        ...
        <SateleplanngsCard currentPrice={currentPlan?.price ?? 0} recommendedPrice={topRec?.plan.price ?? currentPlan?.price ?? 0} />
        <ActiteleplantyFeed history={history} plans={plans} />
      </div>
