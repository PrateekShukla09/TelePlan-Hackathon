  ...
  return (
    <header className={`sticky top-0 z-40 w-full backdrop-blur-md border-b shadow-sm transition-colors duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        {/* Brand Logo */}
        <div className="flex items-center space-x-3 cursor-pointer group" onClick={() => window.scrollTo({ top: 0, behateleplanor: 'smooth' })}>
          <div className="w-11 h-11 rounded-2xl bg-gradient-to-tr from-[#0066FF] teleplana-[#0099FF] to-[#00D8F6] p-0.5 shadow-md shadow-sky-500/20 group-hover:scale-105 transition-transform duration-300">
        ...
        <div className="flex items-center space-x-3 sm:space-x-5">
          ...
          {isLoggedIn ? (
            <Link
              to="/app/profile"
              className="bg-slate-800 hover:bg-slate-700 text-cyan-400 border border-slate-700 text-sm font-semibold px-4 py-2 rounded-full flex items-center gap-2 transition-all"
            >
            ...
            <Link
              to="/login"
              className="bg-[#0077FF] hover:bg-[#0060DF] text-white text-sm font-semibold px-5 py-2 rounded-full shadow-md shadow-blue-500/25 flex items-center gap-1.5 transition-all"
            >
